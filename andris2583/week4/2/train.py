...
data = pd.concat([
    ...
    pd.read_parquet('../../../Translated_Questions/translated_ru_rows.parquet', 
                       columns=['context', 'question', 'answerable', 'answer', 'lang']),
    pd.read_parquet('../../../Translated_Questions/translated_ja_rows.parquet', 
                       columns=['context', 'question', 'answerable', 'answer', 'lang'])
])[:500]
...
model = XLMRobertaForTokenClassification.from_pretrained('xlm-roberta-base', num_labels=2)
...
class WeightedTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss_fct = CrossEntropyLoss(weight=torch.tensor([1, 100.0]).to(device))
